...
ExpressApp.prototype.start = function(opts, cb) {
  ...
  router.get('/v2/proposals/', function(req, res) {
    var opts = {};
    ...
    try {
      server = getServer(req, res);
      server._checkProposalsInBlockchain(function(err, proposals) {
        if (err) return returnError(err, res, req);
        res.json(proposals);
      });
    } catch (ex) {
  ...
  router.get('/v2/masternodes/list/', function(req,res){
	var opts = {};
	if (req.query.coin) opts.coin = req.query.coin;
	if (req.query.network) opts.network = req.query.network;
	...
	try {
		server = getServer(req, res);
		server._checkMasternodeList(function(err, mnList) {
		if (err) return returnError(err, res, req);
		res.json(mnList);
		});
	} catch (ex) {
