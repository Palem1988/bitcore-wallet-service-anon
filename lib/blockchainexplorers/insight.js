...
function Insight(opts) {
  ...
  this.apiPrefix = opts.apiPrefix || 'insight-api-anon';
...
Insight.prototype.getProposals = function(cb) {
  var args = {
    method: 'GET',
    path: this.apiPrefix + '/gobject/list',
    json: true,
  };

  this._doRequest(args, function(err, res, proposals) {
    if (res && res.statusCode == 404) return cb();
    if (err || res.statusCode !== 200)
      return cb(_parseErr(err, res));
...
Insight.prototype.getMasternodeList = function(cb){
	var args = {
		method: 'GET',
		path: this.apiPrefix + '/masternodes/list',
		json: true,
	};

	this._doRequest(args, function(err, res, mnList){
		if (res && res.statusCode == 404) return cb();
		if (err || res.statusCode !== 200)
			return cb(_parseErr(err, res));

		return cb(null, mnList);
	})
}
