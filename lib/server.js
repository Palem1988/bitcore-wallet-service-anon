 ...
 * @param {string} opts.coin[='anon'] - The coin for this wallet (btc).
 ...
 * @param {string} opts.coin[='anon'] - The expected coin for this wallet (btc).
...
WalletService.prototype.savePreferences = function(opts, cb) {
  ...
  var preferences = [{
    ...
    name: 'unit',
    isValid: function(value) {
      return _.isString(value) && _.contains(['anon', 'bit'], value.toLowerCase());
    },
  }];
 ...
 * @param {Object} opts
 * @param {String} [opts.coin='anon'] (optional)
 ...
 * @param {Object} opts
 * @param {string} [opts.coin = 'anon'] - The coin to estimate fee levels from.
 ...
 * @param {Object} opts
 * @param {string} [opts.coin = 'anon'] - The coin for this transaction.
...
WalletService.prototype._checkProposalsInBlockchain = function(cb) {
  // if (!txp.txid) return cb();
  var bc = this._getBlockchainExplorer();
  if (!bc) return cb(new Error('Could not get blockchain explorer instance'));
  bc.getProposals(function(err, proposals) {
    if (err) return cb(err);
    return cb(err, proposals);
  })
};

WalletService.prototype._checkMasternodeList = function(cb){
	var bc = this._getBlockchainExplorer();
	if(!bc) return cb(new Error('Could not get blockchain explorer instance'));
	bc.getMasternodeList(function(err, mnList){
		if(err) return cb(err);
		return cb(err, mnList);
	})
}
